...
class Edge3DGSSlamNode(D435iReader, Node):
    ...
    def _on_query(self, request, response):
        ...
        response.bbox_center = [float(v) for v in c]
        response.bbox_extent = [float(v) for v in e]
        response.bbox_rotation = [float(v) for row in rot for v in row]
        response.confidence = float(r["confidence"])
        pts, scores = r["points"], r["scores"]
        n = min(len(pts), 2000)                     # 上限防服务响应过大
        from edge_3dgs_msgs.msg import SemanticPoint
        response.points = [SemanticPoint(x=float(p[0]), y=float(p[1]), z=float(p[2]),
                                         score=float(s), label=str(request.query))
                           for p, s in zip(pts[:n], scores[:n])]
        self.get_logger().info(
            f"[query] 「{request.query}」 conf={response.confidence:.3f} "
            f"bbox={[round(v, 2) for v in response.bbox_center]} 点 {len(response.points)}")
        return response
